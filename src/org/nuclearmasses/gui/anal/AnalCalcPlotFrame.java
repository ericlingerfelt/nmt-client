...
/**
 * This class is the Mass Dataset Plotting Interface for
 * the Mass Dataset Analyzer > Mass Dataset RMS Calculator.
 */
public class AnalCalcPlotFrame extends JFrame implements ActionListener
															, ItemListener
															, ChangeListener{
	...
	/** The anal calc plot panel. */
	private AnalCalcPlotPanel analCalcPlotPanel;
	
	/** The anal calc table frame. */
	private AnalCalcTableFrame analCalcTableFrame;
	 ...
	 */
	public AnalCalcPlotFrame(AnalDataStructure ds, MainDataStructure mds){
		this.mds = mds;
		this.ds = ds;
		c = getContentPane();
		setSize(950, 725);
	...
	public void closeAllFrames(){
		if(analCalcTableFrame!=null){
			analCalcTableFrame.setVisible(false);
			analCalcTableFrame.dispose();
		}
	}
	 ...
	 */
	protected void setFormatPanelState(boolean resetXValues, boolean resetYValues){
	...
	private void createResultsPlot(){
		analCalcPlotPanel = new AnalCalcPlotPanel(this);
		analCalcPlotPanel.setPreferredSize(analCalcPlotPanel.getSize());
		analCalcPlotPanel.revalidate();
		sp = new JScrollPane(analCalcPlotPanel);
	...
	public void actionPerformed(ActionEvent ae){
		...
		if(ae.getSource()==printButton){
			...
			PlotPrinter.print(new VizPlotPrintable(analCalcPlotPanel), this);
		
		}else if(ae.getSource()==saveButton){
			
			PlotSaver.savePlot(analCalcPlotPanel, this, mds);
			...
			if(analCalcTableFrame==null){
				analCalcTableFrame = new AnalCalcTableFrame(ds, mds, this);
	       	}
			analCalcTableFrame.setTableText(mmds);
			analCalcTableFrame.setVisible(true);
	...
	public void redrawPlot(){
		analCalcPlotPanel.setPlotState();
		analCalcPlotPanel.repaint();
	}
	...
	private double[] getAArray(){
		...
		for(int i=0; i<outputArray.length; i++){
			outputArray[i] = ((Point)(pointVector.elementAt(i))).getX() 
				+ ((Point)(pointVector.elementAt(i))).getY();
		}
		return outputArray;
	}
	...
	private double[] getAArrayRMS(){
		...
		for(int i=0; i<pointVector.size(); i++){
			Integer tempDouble = new Integer((int)((Point)(pointVector.elementAt(i))).getX() 
					+ (int)((Point)(pointVector.elementAt(i))).getY());
...
class VizPlotPrintable implements Printable{
	private AnalCalcPlotPanel panel;
	public VizPlotPrintable(AnalCalcPlotPanel panel){
		this.panel = panel;
	}
...
class ReloadDataTask extends org.nuclearmasses.gui.util.SwingWorker<Void, Void>{
	...
	private AnalCalcPlotFrame frame;
	private DelayDialog dialog;
	
	public ReloadDataTask(AnalCalcPlotFrame frame, DelayDialog dialog){
		this.frame = frame;
		this.dialog = dialog;
	}
	...
	public Void doInBackground(){
		...
		frame.setFormatPanelState(false, false);
		frame.redrawPlot();
		frame.validate();
		return null;
	}
...
class AnalCalcPlotPanel extends JPanel{
	...
	int mouseX = 0;
	int mouseY = 0;
	public AnalCalcPlotFrame frame;

	public AnalCalcPlotPanel(AnalCalcPlotFrame frame){
		this.frame = frame;
		setPlotState();
